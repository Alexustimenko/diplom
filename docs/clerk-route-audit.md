...
Обновление этапа 2: публичная таксономия и навигация теперь загружаются из SQL Server, брендовые URL распознаются отдельно от категорий, а полный справочный аудит и результаты идемпотентной синхронизации находятся в [`docs/clerk-catalog-audit.md`](clerk-catalog-audit.md).
